...
The bitstring module provides four classes, :class:`BitStream`, :class:`BitArray`, :class:`ConstBitStream` and :class:`ConstBitArray`, instances of which can be constructed from integers, floats, hex, octal, binary, strings or files, but they all just represent a string of binary digits. I shall use the general term 'bitstring' when referring generically to any of the classes, and use the class names for parts that apply to only one or another.
...
The different views or interpretations on the data are accessed through properties such as :attr:`~ConstBitArray.hex`, :attr:`~ConstBitArray.bin` and :attr:`~ConstBitArray.int`, and an extensive set of functions is supplied for modifying, navigating and analysing the binary data.
