...
class Reader:
    ...
    bytepos -- The current byte position.
        ...
        if type(dtype) is str:
            ...
            if info is None:
                ...
                if bitlength is not None:
                    tibs_dtype = bitstore.tibs_dtype_for(d._name, bitlength)
    ...
    def _search(self, bs: BitsType, bytealigned: bool,
        ...
        if backwards:
            found = self._bits._bitstore.rfind(needle._bitstore, 0, pos, bytealigned)
        else:
            found = self._bits._bitstore.find(needle._bitstore, pos, len(self._bits), bytealigned)
    ...
    def seek_to(self, bs: BitsType, /, *, bytealigned: bool = False) -> bool:
        ...
        """
        found, _ = self._search(bs, bytealigned)
        if found is None:
            return False
        self._pos = found
        return True

    def seek_past(self, bs: BitsType, /, *, bytealigned: bool = False) -> bool:
        ...
        """
        found, needle_length = self._search(bs, bytealigned)
        if found is None:
            return False
        self._pos = found + needle_length
        return True

    def seek_back_to(self, bs: BitsType, /, *, bytealigned: bool = False) -> bool:
        ...
        """
        found, _ = self._search(bs, bytealigned, backwards=True)
        if found is None:
            return False
        self._pos = found
        return True
    ...
    def read_past(self, bs: BitsType, /, *, bytealigned: bool = False) -> Bits:
