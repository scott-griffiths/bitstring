...
class Dtype:
    ...
    __slots__ = ('read_fn', 'name', 'set_fn', 'get_fn', 'return_type', 'is_signed', 'creation_needs_length', 'bitlength', 'bits_per_item', 'length')
    ...
    def create(cls, definition: DtypeDefinition, length: Optional[int]) -> Dtype:
        ...
        x.creation_needs_length = definition.creation_needs_length
        if not x.creation_needs_length or len(dtype_register.names[x.name].allowed_lengths) == 1:
    ...
    def __str__(self) -> str:
        hide_length = not self.creation_needs_length or len(dtype_register.names[self.name].allowed_lengths) == 1 or self.length is None
...
class DtypeDefinition:
    ...
    def __init__(self, name: str, set_fn, get_fn, return_type: Any = Any, is_signed: bool = False, bitlength2chars_fn = None,
                 creation_needs_length: bool = True, allowed_lengths: Tuple[int, ...] = tuple(), multiplier: int = 1, description: str = ''):

        # Consistency checks
        # if creation_needs_length and allowed_lengths:
        ...
        self.creation_needs_length = creation_needs_length
        ...
        if not self.creation_needs_length:
    ...
    def __repr__(self) -> str:
        ...
        s += f"is_signed={self.is_signed}, creation_needs_length={self.creation_needs_length}, allowed_lengths={self.allowed_lengths!s}, multiplier={self.multiplier})"
        return s
...
class Register:
    ...
    def __repr__(self) -> str:
        s = [f"{'key':<12}:{'name':^12}{'signed':^8}{'creation_needs_length':^23}{'allowed_lengths':^16}{'multiplier':^12}{'return_type':<13}"]
        s.append('-' * 85)
        for key in self.names:
            m = self.names[key]
            allowed = '' if not m.allowed_lengths else m.allowed_lengths
            ret = 'None' if m.return_type is None else m.return_type.__name__
            s.append(f"{key:<12}:{m.name:>12}{m.is_signed:^8}{m.creation_needs_length:^16}{allowed!s:^16}{m.multiplier:^12}{ret:<13} # {m.description}")
        return '\n'.join(s)
