...
# When uncompressed this gives the conversion from every possible float16 value to a p4binary value.
lut_float16_to_p4binary_compressed = b"x\x01\xed\xdde\xb6\x96\x05\x00E\xe1\x8f\xee\x06\xe9FZA\xa4\xbb\xbb;\xa4SB\xba\xeb\xd2\xdd\x8dt\x97\x92J(\xa14\xa2\x84" \
...
# When uncompressed this gives the conversion from every possible float16 value to a p3binary value.
lut_float16_to_p3binary_compressed = b'x\x01\xed\xdde\xa2\x16T\x00E\xd1\x07\n\x82R\x92J\nJHK7"\xa1H#\xd2HIwwI\xa7R\xd2H\xab(\xdd%(\xadt\x89A\xa7\xd2\xdd1\x8c' \
...
# When uncompressed this maps each single byte integer to the Python float value that it represents when interpreted as a p4binary value.
lut_int8_to_p4binary_compressed = b'x\x01\x15\xcc[\xb5\x90!\x10\x80Q"\x18\x81\x08<\xabGQ\x0b\x10\x81\x084\x90\x08D \x02\xcf^\xd1S\xe0\x8f@\x04"\xb8e\xad=/3\x1f!\xfc\x7f\xfd\xad\xf1.\x84Lg\xb29' \
...
# When uncompressed this maps each single byte integer to the Python float value that it represents when interpreted as a p3binary value.
lut_int8_to_p3binary_compressed = b'x\x01\x1d\xca\xd9\x11\x10\x06\x08EQJ\xb1\x0b\x8d[\xd0\xb8\xb4A\'R\n]\xc45\xc1\xb8\xb5A)\x9e\t3\xe7\x87\xfb"\xfe\xbf\x87\x11\xcd\x12\x8f"\x1e\x90\x14\xcd\xb0\x1c' \
...
class FP8Format:
    ...
    def __init__(self, exp_bits: int, bias: int):
        ...
        if exp_bits == 4 and bias == 8:
            self.lut_float16_to_float8 = zlib.decompress(lut_float16_to_p4binary_compressed)
            self.lut_int8_to_float = struct.unpack('<256f', zlib.decompress(lut_int8_to_p4binary_compressed))
        elif exp_bits == 5 and bias == 16:
            self.lut_float16_to_float8 = zlib.decompress(lut_float16_to_p3binary_compressed)
            self.lut_int8_to_float = struct.unpack('<256f', zlib.decompress(lut_int8_to_p3binary_compressed))
        else:  # pragma: no cover
            raise RuntimeError("Unsupported float8 format trying to be created. Only p4binary and p3binary available for now.")
            ...
            # lut_float16_to_p4binary_compressed = zlib.compress(self.lut_float16_to_float8, 1)
