...
class Bits:
    ...
    def _read_dtype_list(self, dtypes: List[Dtype], pos: int) -> Tuple[List[Union[int, float, str, Bits, bool, bytes, None]], int]:
        ...
        for dtype in dtypes:
            stretchy = dtype.bitlength is None and dtype.creation_needs_length
            if stretchy:
                ...
                if not dtype.creation_needs_length:
    ...
    def _pp(self, dtype1: Dtype, dtype2: Optional[Dtype], bits_per_group: int, width: int, sep: str, format_sep: str,
        ...
        if not dtype1.creation_needs_length:
            raise ValueError(f"Can't use Dtype '{dtype1}' in pp() as it has an unknown length.")
        if dtype2 is not None and not dtype2.creation_needs_length:
            raise ValueError(f"Can't use Dtype '{dtype1}' in pp() as it has an unknown length.")
        offset_width = 0
