...
def uie2bitstore(i: Union[str, int]) -> BitStore:
    ...
    return BitStore.from_binary_string('0b1' if i == 0 else '0b0' + '0'.join(bin(i + 1)[3:]) + '1')


def sie2bitstore(i: Union[str, int]) -> BitStore:
    i = int(i)
    if i == 0:
        return BitStore.from_binary_string('0b1')
    else:
        return uie2bitstore(abs(i)) + (BitStore.from_binary_string('0b1') if i < 0 else BitStore.from_binary_string('0b0'))
...
def e8m0mxfp2bitstore(f: Union[str, float]) -> BitStore:
    f = float(f)
    if math.isnan(f):
        return BitStore.from_binary_string('0b11111111')
    try:
...
def mxint2bitstore(f: Union[str, float]) -> BitStore:
    ...
    if f > 127:  # 1 + 63/64
        return BitStore.from_binary_string('0b01111111')
    if f <= -128:  # -2
        return BitStore.from_binary_string('0b10000000')
