...
import bitstring
from bitstring import BitStream, ConstBitStream, offsetcopy, pack
...
class Pack(unittest.TestCase):
    def testPack1(self):
        s = bitstring.pack(
            "uint:6, bin, hex, int:6, se, ue, oct", 10, "0b110", "ff", -1, -6, 6, "54"
        )
    ...
    def testPackWithDict4(self):
        ...
        t = BitStream(
            "int:8=1, 0b1, int:8=2, 0b1, int:8=3, 0b1, int:8=4, 0b1, int:8=5, 0b1"
        )
        self.assertEqual(s, t)
    ...
    def testPackWithLengthRestriction(self):
        ...
        self.assertRaises(
            bitstring.CreationError, pack, "bits:12=b", b=BitStream("0b11")
        )
...
class FromFile(unittest.TestCase):
    ...
    def testFileProperties(self):
        ...
        self.assertEqual(
            s.bin, "0000000100100011010001010110011110001001101010111100110111101111"
        )
    ...
    def testCreationFromFileWithLength(self):
        ...
        self.assertFalse(s)
        self.assertRaises(
            bitstring.CreationError, BitStream, filename="test/smalltestfile", length=65
        )
        self.assertRaises(
            bitstring.CreationError,
            ConstBitStream,
            filename="test/smalltestfile",
            length=64,
            offset=1,
        )
        ...
        with open("test/smalltestfile", "rb") as f:
            ...
            self.assertRaises(
                bitstring.CreationError, ConstBitStream, auto=f, length=65
            )
            self.assertRaises(
                bitstring.CreationError, ConstBitStream, auto=f, offset=60, length=5
            )
...
class Split1(unittest.TestCase):
    ...
    def testSplitByteAlignedWithIntialBytes(self):
        ...
        self.assertEqual(
            [b.hex for b in bsl],
            ["aa", "471234fedc43", "47112233", "47", "4723", "472314"],
        )
        self.assertEqual(s.bytepos, 1)
...
class Adding(unittest.TestCase):
    ...
    def testWritingData(self):
        strings = [
            BitStream(bin=x) for x in ["0", "001", "0011010010", "010010", "1011"]
        ]
        s = BitStream().join(strings)
...
class Multiplication(unittest.TestCase):
    ...
    def testFileAndMemEquivalence(self):
        ...
        self.assertEqual(
            a._datastore.getbyteslice(1, 5), bytearray(b._datastore.getbyteslice(1, 5))
        )
...
class Split2(unittest.TestCase):
    ...
    def testSplitEndbit(self):
        ...
        self.assertEqual(
            list(a[4:12].split("0b0", False)), list(a.split("0b0", start=4, end=12))
        )
    ...
    def testRepr(self):
        ...
        for bs in bls:
            ...
            self.assertTrue(a == b)
        for f in [
            ConstBitStream(filename="test/test.m1v"),
            ConstBitStream(filename="test/test.m1v", length=17),
            ConstBitStream(filename="test/test.m1v", length=23, offset=23102),
        ]:
            f2 = eval(f.__repr__())
            self.assertEqual(
                f._datastore._rawarray.source.name, f2._datastore._rawarray.source.name
            )
        ...
        a += "0xf"
        self.assertEqual(
            repr(a), "BitStream('0x" + "f" * max + "...') # length=%d" % (max * 4 + 4)
        )
    ...
    def testIntelligentRead5(self):
        ...
        self.assertEqual(
            (c0, c1, c2), (BitStream("0x00"), BitStream("0x11"), BitStream("0x2233"))
        )
        a.pos = 0
        c = a.read("bits:16")
    ...
    def testTokenParser(self):
        ...
        self.assertEqual(
            tp("2*bits:6"), (False, [("bits", 6, None), ("bits", 6, None)])
        )
...
class AllAndAny(unittest.TestCase):
    ...
    def testFloatErrors(self):
        ...
        for i in (8, 10, 12, 18, 30, 128, 200):
            self.assertRaises(ValueError, BitStream, float=1.0, length=i)
...
class Bugs(unittest.TestCase):
    def testBugInReplace(self):
        s = BitStream("0x00112233")
        a = list(s.split("0x22", start=8, bytealigned=True))
        self.assertEqual(a, ["0x11", "0x2233"])
        s = BitStream("0x00112233")
    ...
    def testFunctionNegativeIndices(self):
        ...
        a = list(s.findall("0x1", bytealigned=True, start=-15))
        self.assertEqual(a, [24])
        a = list(s.findall("0x1", bytealigned=True, start=-16))
        self.assertEqual(a, [16, 24])
        a = list(s.findall("0x1", bytealigned=True, end=-5))
        self.assertEqual(a, [0, 16])
        a = list(s.findall("0x1", bytealigned=True, end=-4))
        self.assertEqual(a, [0, 16, 24])
        ...
        # cut
        s = BitStream("0x12345")
        a = list(s.cut(4, start=-12, end=-4))
        self.assertEqual(a, ["0x3", "0x4"])

        # split
        s = BitStream("0xfe0012fe1200fe")
        a = list(s.split("0xfe", bytealigned=True, end=-1))
        self.assertEqual(a, ["", "0xfe0012", "0xfe1200f, 0b111"])
        a = list(s.split("0xfe", bytealigned=True, start=-8))
        self.assertEqual(a, ["", "0xfe"])
    ...
    def testBracketExpander(self):
        ...
        self.assertEqual(
            be("2*(a, b, 3*(c, d), e)"), "a,b,c,d,c,d,c,d,e,a,b,c,d,c,d,c,d,e"
        )
    ...
    def testPackCodeDicts(self):
        self.assertEqual(
            sorted(bitstring.REPLACEMENTS_BE.keys()),
