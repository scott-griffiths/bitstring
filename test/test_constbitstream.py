...
class CreationWithPos(unittest.TestCase):
    ...
    def testStringRepresentationFromFile(self):
        s = CBS(filename="test/test.m1v", pos=2001)
        self.assertEqual(
            s.__repr__(),
            "ConstBitStream(filename='test/test.m1v', length=1002400, pos=2001)",
        )
        s.pos = 0
        self.assertEqual(
            s.__repr__(), "ConstBitStream(filename='test/test.m1v', length=1002400)"
        )
