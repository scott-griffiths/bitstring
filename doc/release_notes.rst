...
This affects the methods ``slice``, ``find``, ``findall``, ``rfind``, ``reverse``, ``cut`` and ``split``.

The parameter 'bitpos' has been renamed to 'pos'. The affects the methods
...
The ``pack`` function provides a flexible new method for creating
...
The ``unpack`` method is similar to the ``read`` method except that it always
...
The ``tostring`` method just returns the data as a string, with up to seven
zero bits appended to byte align. The ``tofile`` method does the same except
...
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
The special methods for ``hex`` and ``oct`` have been removed. Please use the
...
now also use the peek method with ue and se.
...
This means that a step can also be given to the slice method so that the
...
Added ``rfind``, ``findall`` and ``replace``. These do pretty much what you'd expect -
see the docstrings or the wiki for more information.

More special methods
^^^^^^^^^^^^^^^^^^^^

Some missing methods were added: ``__repr__``, ``__contains__``, ``__rand__``,
...
A number of methods (especially ``find`` and ``findbytealigned``) have been sped
up considerably.
...
Added ``seekbit`` and ``seekbyte`` methods. These complement the 'advance' and
...
Some missing methods such as ``advancebit`` and ``deletebytes`` have been added. Also
a number of 'peek' methods make an appearance as have ``prepend`` and ``reversebits``.
