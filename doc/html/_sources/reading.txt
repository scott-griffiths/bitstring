...
In order to behave like a file or stream, every bitstring has a property :attr:`~Bits.pos` which is the current position from which reads occur. :attr:`~Bits.pos` can range from zero (its value on construction) to the length of the bitstring, a position from which all reads will fail as it is past the last bit.

The property :attr:`~Bits.bytepos` is also available, and is useful if you are only dealing with byte data and don't want to always have to divide the bit position by eight. Note that if you try to use :attr:`~Bits.bytepos` and the bitstring isn't byte aligned (i.e. :attr:`~Bits.pos` isn't a multiple of 8) then a :exc:`Error` exception will be raised.
...
The :meth:`~Bits.unpack` method works in a very similar way to :meth:`~Bits.readlist`. The major difference is that it interprets the whole bitstring from the start, and takes no account of the current :attr:`~Bits.pos`. It's a natural complement of the :func:`pack` function. ::
...
The properties :attr:`~Bits.pos` and :attr:`~Bits.bytepos` are available for getting and setting the position, which is zero on creation of the bitstring.

Note that you can only use :attr:`~Bits.bytepos` if the position is byte aligned, i.e. the bit position is a multiple of 8. Otherwise a :exc:`ByteAlignError` exception is raised.
