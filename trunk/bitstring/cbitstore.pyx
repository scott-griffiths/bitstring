...
cdef class MmapByteArray:
    """Looks like a bytearray, but from an mmap."""

    #__slots__ = ('filemap', 'filelength', 'source', 'byteoffset', 'bytelength')
    cdef public int filelength, byteoffset, bytelength
    cdef public object filemap, source

    def __init__(self, source, bytelength=-1, byteoffset=-1):
        self.source = source
...
cdef class ConstByteArray:
    """Stores raw bytes together with a bit offset and length."""

    #__slots__ = ('offset', '_rawarray', 'bitlength')
    cdef public int offset, bitlength
    ...
    def _appendarray(self, ConstByteArray array):
...
cdef class ByteArray(ConstByteArray):
    #__slots__ = ()
...
cpdef ConstByteArray offsetcopy(ConstByteArray s, int newoffset):
    """Return a copy of s with the newoffset."""
    assert 0 <= newoffset < 8
    cdef long shiftleft, shiftright, x, bits_in_last_byte
    if not s.bitlength:
        return copy.copy(s)
    else:
