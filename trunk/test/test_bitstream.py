...
class ModuleData(unittest.TestCase):
    ...
    def testReverseDict(self):
        ...
        for i in range(256):
            ...
            b = d[i]
            self.assertEqual(a.bin[2:][::-1], BitStream(bytes=b).bin[2:])
...
class Creation(unittest.TestCase):
    ...
    def testCreationFromHexErrors(self):
        self.assertRaises(bitstring.CreationError, BitStream, hex='0xx0')
        self.assertRaises(bitstring.CreationError, BitStream, hex='0xX0')
        self.assertRaises(bitstring.CreationError, BitStream, hex='0Xx0')
        self.assertRaises(bitstring.CreationError, BitStream, hex='-2e')

    def testCreationFromBin(self):
        s = BitStream(bin='1010000011111111')
        self.assertEqual((s.length, s.hex), (16, '0xa0ff'))
        self.assertEqual(s.bitpos, 0)
        s = BitStream(bin='00')[:1]
        self.assertEqual(s.bin, '0b0')
        s = BitStream(bin=' 0000 \n 0001\r ')
    ...
    def testCreationFromUint(self):
        s = BitStream(uint = 15, length=6)
        self.assertEqual(s.bin, '0b001111')
        s = BitStream(uint = 0, length=1)
        self.assertEqual(s.bin, '0b0')
        s.uint = 1
        self.assertEqual(s.uint, 1)
        s = BitStream(length=8)
        s.uint = 0
    ...
    def testCreationFromUintWithOffset(self):
        self.assertRaises(bitstring.Error, BitStream, uint=12, length=8, offset=1)

    def testCreationFromUintErrors(self):
        self.assertRaises(bitstring.CreationError, BitStream, uint=-1, length=10)
        self.assertRaises(bitstring.CreationError, ConstBitStream, uint=12)
        self.assertRaises(bitstring.CreationError, BitStream, uint=4, length=2)
        self.assertRaises(bitstring.CreationError, BitStream, uint=0, length=0)
        self.assertRaises(bitstring.CreationError, BitStream, uint=12, length=-12)

    def testCreationFromInt(self):
        s = BitStream(int=0, length=4)
        self.assertEqual(s.bin, '0b0000')
        s = BitStream(int=1, length=2)
        self.assertEqual(s.bin, '0b01')
        s = BitStream(int=-1, length=11)
        self.assertEqual(s.bin, '0b11111111111')
        s = ConstBitStream(int=12, length=7)
        self.assertEqual(s.int, 12)
        s = BitStream(int=-243, length=108)
        self.assertEqual((s.int, s.length), (-243, 108))
        for length in range(6, 10):
            for value in range(-17, 17):
                s = BitStream(int=value, length=length)
                self.assertEqual((s.int, s.length), (value, length))
        s = BitStream(int=10, length=8)
        s.int = 20
        self.assertEqual(s.int, 20)
        self.assertEqual(s.length, 8)

    def testCreationFromIntErrors(self):
        self.assertRaises(bitstring.CreationError, BitStream, int=-1, length=0)
        self.assertRaises(bitstring.CreationError, BitStream, int=12)
        self.assertRaises(bitstring.CreationError, BitStream, int=4, length=3)
        self.assertRaises(bitstring.CreationError, BitStream, int=-5, length=3)
    ...
    def testCreationFromSeWithOffset(self):
        self.assertRaises(bitstring.CreationError, BitStream, se=-13, offset=1)

    def testCreationFromSeErrors(self):
        self.assertRaises(bitstring.CreationError, BitStream, se=-5, length=33)
        s = BitStream(bin='001000')
        self.assertRaises(bitstring.InterpretError, s._getse)

    def testCreationFromUe(self):
        [self.assertEqual(BitStream(ue=i).ue, i) for i in range(0, 20)]

    def testCreationFromUeWithOffset(self):
        self.assertRaises(bitstring.CreationError, BitStream, ue=104, offset=2)

    def testCreationFromUeErrors(self):
        self.assertRaises(bitstring.CreationError, BitStream, ue=-1)
        self.assertRaises(bitstring.CreationError, BitStream, ue=1, length=12)
        s = BitStream(bin='10')
